...
/* Subject catalogue shared by the subjects page and the statistics page.
 * The full definitions live in the "Subjects page" section further down. */
static const char *sub_keys[NUM_SUBJECTS];
static const int sub_unit_start[NUM_SUBJECTS];
static const int sub_unit_count[NUM_SUBJECTS];
...
static void net_slide_apply(void);
static void net_rail_theme_reset(void);
...
static char *build_theme_css(const ThemePalette *p) {
    return g_strdup_printf(
        ...
        "   border-radius: 24px;"
        "   padding: 24px 34px;"
        "   box-shadow: 0 10px 30px alpha(@bg_crust, 0.20);"
        "}"
        ".notes-kicker {"
        "   color: @accent2;"
        "   font-weight: 800;"
        ...
        "   font-weight: 700;"
        "}"
        ".stats-subject-count.locked {"
        "   color: @fg_overlay;"
        "}"
        ".stats-subject-bar,"
        ".stats-subject-bar trough {"
        "   min-height: 12px;"
        "   border-radius: 999px;"
        "   background-color: alpha(@bg_surface1, 0.85);"
        "   border: none;"
        ...
        ".stats-units {"
        "   margin-top: 8px;"
        "   border-left: 2px solid alpha(@fg_overlay, 0.22);"
        "   padding-left: 12px;"
        "}"
        ".stats-unit-row {"
...
static void draw_wifi_icon(GtkDrawingArea *area, cairo_t *cr,
                           int width, int height, gpointer data) {
...
static void progress_for_units(int first, int n, ProgressSum *out) {
    for (int j = 0; j < n; j++) {
        UnitCtx *u;
        int d;
...
static GtkWidget *make_unit_stats_row(UnitCtx *u, int num, int global) {
    GtkWidget *row;
    GtkWidget *head;
    GtkWidget *left;
    GtkWidget *idx;
...
static GtkWidget *net_slide_stack;
static GtkWidget *net_prev_btn;
static GtkWidget *net_next_btn;
static guint net_slide_idx;

static void net_paragraph(GtkWidget *box, const char *text) {
...
static void net_heading(GtkWidget *box, const char *text) {
    GtkWidget *l = gtk_label_new(text);
...
static GtkWidget *net_note_host;
static GArray *net_note_kicks;
static GArray *net_note_titles;
static GArray *net_note_bodies;
static int net_note_last_w = -1;

static void net_note_font(GtkWidget *l, int px) {
...
static void net_rescale_notes(void) {
...
static gboolean net_note_tick(GtkWidget *w, GdkFrameClock *clock,
                              gpointer data) {
...
static GtkWidget *net_slide_card(const char *cls) {
...
static void net_note_kicker(GtkWidget *box, const char *text) {
...
static void net_note_title(GtkWidget *box, const char *text) {
...
static void net_note_line(GtkWidget *box, const char *text, gboolean tip) {
...
static void net_open_unit(GtkButton *button, gpointer data) {
    (void)button;
    (void)data;
    net_slide_idx = 0;
    net_slide_apply();
...
static void net_slide_prev(GtkButton *button, gpointer data) {
    (void)button;
    (void)data;
    if (net_slide_idx > 0) {
        net_slide_idx--;
...
static void net_slide_next(GtkButton *button, gpointer data) {
    (void)button;
    (void)data;
...
static GtkWidget *net_scroll;
static GtkWidget *net_fixed;
static GtkWidget *net_rail;
static GtkWidget *net_nodes[NET_UNITS];
static GtkWidget *net_labels[NET_UNITS];
static double net_cx[NET_UNITS];
static double net_cy[NET_UNITS];
static int net_cols = 1;
static int net_rows = 1;
static int net_cw = (int)(2.0 * ROAD_MX + (NET_UNITS - 1) * PATH_SPAC);
static int net_ch = (int)(2.0 * ROAD_MY);
static guint net_idle;
static double net_last_avail = -1.0;
static int net_last_cols = -1;
static int net_last_rows = -1;
static int net_last_cw = -1;
static int net_last_ch = -1;
static cairo_surface_t *net_rail_cache;
static int net_cache_w;
static int net_cache_h;

static void net_point(double t, double *ox, double *oy) {
    int k = (int)t;
    double u = t - (double)k;
    double u2, u3;
    double p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y;
...
static void net_path(cairo_t *cr, double t0, double t1) {
...
static void net_geometry(double avail) {
...
static void net_apply_layout(void) {
    if (!net_fixed)
        return;
...
static gboolean net_relayout_idle(gpointer data) {
    (void)data;
    net_idle = 0;
    net_relayout();
    return G_SOURCE_REMOVE;
...
static void net_relayout_later(void) {
    if (net_idle == 0)
        net_idle = g_idle_add(net_relayout_idle, NULL);
}

static void net_adjust_notify(GtkAdjustment *adj, GParamSpec *ps,
                              gpointer data) {
    (void)adj;
    (void)ps;
    (void)data;
...
static void net_render_rail_to(cairo_t *cr) {
...
static void net_draw_rail(GtkDrawingArea *area, cairo_t *cr,
                          int width, int height, gpointer data) {
    (void)area;
    (void)width;
    (void)height;
...
static void net_add_node(GtkFixed *fixed, int index) {
    int num = index + 1;
...
static GtkWidget *build_netmap_page(void) {
    GtkWidget *page;
    GtkWidget *scroll;
    GtkWidget *wrap;
    GtkWidget *fixed;
...
static GtkWidget *build_net_unit1_page(void) {
    typedef struct {
        const char *kicker;
        const char *title;
        const char *tip;
...
static const NetTask net_tasks[] = {
    {
...
static const NetAns net_ans[4][4] = {
    { /* 10.0.0.0/24 */
...
static GtkWidget *net_qz_combo(void) {
...
static void net_qz_set_prefix(GtkComboBoxText *c, int pfx) {
    if (pfx >= 8 && pfx <= 30)
...
static int net_qz_prefix(GtkComboBoxText *c) {
...
static gboolean net_txt_eq(const char *a, const char *b) {
    const char *x, *y;
...
static gboolean net_qz_entry_ok(GtkWidget *entry, const char *want) {
    const gchar *got = gtk_editable_get_text(GTK_EDITABLE(entry));

    return got && net_txt_eq(got, want);
}

static void net_qz_check(GtkButton *button, gpointer data) {
    NetQz *q = data;
    int ok = 0;
...
static void net_qz_fill(GtkButton *button, gpointer data) {
    NetQz *q = data;
...
static void net_qz_header(GtkWidget *box, const char *text) {
    GtkWidget *l = gtk_label_new(text);
...
static GtkWidget *net_qz_make_entry(void) {
    GtkWidget *e = gtk_entry_new();
...
static GtkWidget *build_net_exercise_page(void) {
    GtkWidget *page;
    GtkWidget *scroll;
    GtkWidget *body;
