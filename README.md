...
- **Subjects ("Předměty")** – adaptive serpentine subject map with 13 subjects
  ...
  - **Správa počítačových sítí / Computer Network Administration** is open too
    ...
    **units 1–26 are playable**, units 27–30 are locked placeholders
...
- **Computer networks path** – twenty-six unlocked units (theory slides + practice):
  ...
  18. Switch (podrobně) – slides + quiz
  19. Směrování (Routing) – slides + quiz
  20. Směrovač a brána – slides + quiz
  21. Adresy a rámce v ethernetu – slides + quiz
  22. Paket IP – slides + quiz
  23. ICMP – slides + quiz
  24. ARP – slides + quiz
  25. TCP a UDP – slides + quiz
  26. Aplikační protokoly – slides + quiz  
...
4. Click the **"Neue Freunde"** node to open unit 1, **"Aus aller Welt"** for
   ...
   finished yet.
5. On the networks path, units **1–26** are open. Unit 1 (**Výpočet IP adres**)
   walks through four VLSM slides and then four interactive subnetting tasks
   with check / “show solution”. Units 2–26 each have theory slides followed by
   a multiple-choice quiz (basic concepts through application protocols).
   Units 27–30 stay locked placeholders.
...
- **Předměty** – přizpůsobivá „hadí“ mapa s 13 předměty
  ...
  - **Správa počítačových sítí** je také otevřená (ikona Wi‑Fi) a vede na cestu
    s 30 jednotkami; **jednotky 1–26 jsou hratelné**, 27–30 jsou zamčené
    placeholdery
...
- **Cesta počítačových sítí** – šestadvacet odemčených jednotek (snímky + cvičení):
  ...
  18. Switch (podrobně) – snímky + kvíz
  19. Směrování (Routing) – snímky + kvíz
  20. Směrovač a brána – snímky + kvíz
  21. Adresy a rámce v ethernetu – snímky + kvíz
  22. Paket IP – snímky + kvíz
  23. ICMP – snímky + kvíz
  24. ARP – snímky + kvíz
  25. TCP a UDP – snímky + kvíz
  26. Aplikační protokoly – snímky + kvíz  
...
5. Na síťové cestě jsou otevřené jednotky **1–26**. Jednotka 1 (**Výpočet IP
   adres**) má 4 snímky VLSM a pak 4 interaktivní úlohy s kontrolou /
   „ukázat řešení“. Jednotky 2–26 mají snímky teorie a kvíz s výběrem
   (od základních pojmů po aplikační protokoly). Jednotky 27–30 zůstávají zamčené.
...
net.c                   cesta počítačových sítí (30 uzlů), jednotky 1–26
                        (1: VLSM cvičení; 2–26: kvízy s výběrem)
