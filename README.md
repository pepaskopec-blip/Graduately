...
- **Subjects ("Předměty")** – adaptive serpentine subject map with 13 subjects
  ...
  - **Správa počítačových sítí / Computer Network Administration** is open too
    ...
    **units 1–20 are playable**, units 21–30 are locked placeholders
...
- **Computer networks path** – twenty unlocked units (theory slides + practice):
...
4. Click the **"Neue Freunde"** node to open unit 1, **"Aus aller Welt"** for
   ...
   finished yet.
5. On the networks path, units **1–20** are open. Unit 1 (**Výpočet IP adres**)
   walks through four VLSM slides and then four interactive subnetting tasks
   with check / “show solution”. Units 2–20 each have theory slides followed by
   a multiple-choice quiz (basic concepts through routers and gateways).
   Units 21–30 stay locked placeholders.
...
- **Předměty** – přizpůsobivá „hadí“ mapa s 13 předměty
  ...
  - **Správa počítačových sítí** je také otevřená (ikona Wi‑Fi) a vede na cestu
    s 30 jednotkami; **jednotky 1–20 jsou hratelné**, 21–30 jsou zamčené
    placeholdery
...
- **Cesta počítačových sítí** – dvacet odemčených jednotek (snímky + cvičení):
...
5. Na síťové cestě jsou otevřené jednotky **1–20**. Jednotka 1 (**Výpočet IP
   adres**) má 4 snímky VLSM a pak 4 interaktivní úlohy s kontrolou /
   „ukázat řešení“. Jednotky 2–20 mají snímky teorie a kvíz s výběrem
   (od základních pojmů po směrovač a bránu). Jednotky 21–30 zůstávají zamčené.
...
net.c                   cesta počítačových sítí (30 uzlů), jednotky 1–20
                        (1: VLSM cvičení; 2–20: kvízy s výběrem)
